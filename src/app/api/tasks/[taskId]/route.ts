...
const updateTaskSchema = z.object({
  ...
  deadline: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Invalid deadline format (YYYY-MM-DD)').optional(),
...
export async function PATCH(
  ...
  try {
    ...
    const { title, description, assigneeId, priority, startDate, deadline, status, dependencies } = updateTaskSchema.parse(body);
    ...
    const isGuest = !(await hasProjectPermission(userId, module.projectId.toString(), 'Developer'));
    const isTaskAssignee = task.assigneeId.toString() === userId;
...
export async function DELETE(
  ...
  try {
    ...
    const hasPermission = await hasProjectPermission(userId, module.projectId.toString(), 'Developer');
    if (!hasPermission) {
      return NextResponse.json(
